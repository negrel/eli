...
function generate_squashfs {
  ...
  buildah_run -t --user root $1 /eli/scripts/mksquashfs > >(log_pipe "[MKSQUASHFS] %s") 2>&1
  log_info "squashfs generated in container \"$1\"."
}

function generate_initramfs {
  log_info "generating initramfs in \"$1\"..."
  buildah_run -t --user root $1 /eli/scripts/mkinitramfs > >(log_pipe "[MKINITRAMFS] %s") 2>&1
